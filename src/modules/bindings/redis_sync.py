"""
Binding Sync Utilities.

Handles syncing subscription data to Redis after binding changes.
"""
...
async def sync_user_subscriptions_to_redis(user_id: int) -> int:
    """
    Sync a specific user's subscriptions to Redis after binding change.

    This updates the Redis cache with the latest binding info (service_id)
    ...
    try:
        ...
        # Get all enabled subscriptions (with binding info)
        ...
        sync_log.info(
            f"Synced subscriptions to Redis after binding change for user {user_id} "
