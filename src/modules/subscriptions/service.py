...
async def set_source_enabled(
    ...
    """Toggle one source for a subscription; applies source->enabled coupling.
    ...
    Returns:
        The updated subscription row (with enabled + disabled_sources), or None.
    """
    before_enabled = existing["enabled"]
    updated = await repo.set_source_enabled(
        existing["id"], source, enabled, registry.source_keys()
    )
    if not updated:
        return None
    was_disabled = (not before_enabled) and updated["enabled"]
    await _sync_and_maybe_notify(repo, existing["id"], was_disabled)
    return updated
